...
PUBLIC int is_cross(int fd, int n,int count)//n是扇区数，初始值传 pin->i_nr_sects
{
	struct inode * pin = filp[fd]->fd_inode;
	int len = filp[fd]->fd_inode.i_size;//已有大小
...
PUBLIC int copy_and_paste(char*filename1, char*filename2,void*buf, int count)
{
	...
	while (is_cross(fd,n, count))
	{
		mutiple++;
		n = n*mutiple;
	}//问题：相同路径创建原扇区multiple倍的文件，返回一个fd1,
	...
	/*read*/
	int fd2;
	fd2 = open(filename1, O_CREAT | O_RDWR);//filename1,创建后的名字
	assert(fd2 != -1);
	...
	filename3 = filename1;
	filename1 = filename2;
	filename2 = filename3;//问题：名字可以这么交换吗我觉得有点问题
	unlink(pathname);//问题：pathname是第一个文件的路径，如何得到
	//改名，将新文件命名为filename
