...
PRIVATE struct inode * create_folder(char * path, int flags);
PRIVATE int alloc_imap_bit(int dev);
PRIVATE int alloc_smap_bit(int dev, int nr_sects_to_alloc);
PRIVATE struct inode * new_inode(int dev, int inode_nr, int start_sect);
PRIVATE struct inode * new_inode_folder(int dev, int inode_nr, int start_sect)
...
PUBLIC int do_open()
{
	...
	else if (flags&CREAT_FOLDER)
	{
		if (inode_nr) {
			printl("folder exists.\n");
			return -1;
		}
		else {
...
PRIVATE struct inode * create_folder(char * path, int flags)
{
	...
	struct inode *newino = new_inode_folder(dir_inode->i_dev, inode_nr,
		free_sect_nr, 0);
